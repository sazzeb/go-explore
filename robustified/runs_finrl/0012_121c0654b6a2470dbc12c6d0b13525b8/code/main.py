...
def _run(
    ...
    elif isinstance(args.game, str) and args.game.startswith("finrl:"):
        from finrl_goexplore.make_env import FinRLGoExploreEnvFromId
        game_class = FinRLGoExploreEnvFromId
        game_args = dict(env_id=args.game, seed=getattr(args, "seed", 0))
        IMPORTANT_ATTRS = ['score']
        grid_resolution = (GridDimension('score', 1),)
    else:
